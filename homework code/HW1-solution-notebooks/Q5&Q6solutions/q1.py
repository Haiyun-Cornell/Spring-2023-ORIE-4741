...
def perceptron_algo(features, labels, N_iters, geom_margins=[],
    ...
    for _ in range(N_iters):
        ...
        for x, y in zip(features, labels):
            ...
            # TODO:
            # Q1(b): implement margin perceptron
            # now we need to consider margin mistakes
            # change 'mistake' accordingly
            if margin is not None:
                threshold = margin * np.linalg.norm(theta) / 2.0
                mistake = mistake or (np.abs(prediction) < threshold)

            if mistake:
                # update when there is a disagreement
                update = y * x
                # TODO:
                # Q1(c): apply variable increment
                update *= (1/count if variable_increment else 1)
                
                # TODO:
                # Q1(d): apply batch update
                if batch:
                    aggregated += update
                else:
                    theta += update
        ...
        # TODO:
        # Q1(d): update if batch increment
        if batch:
            theta += aggregated

        # TODO:
        # Q1(a): compute the geometric margin
        norm = np.linalg.norm(theta)
        geom_margin = min(agreements) / norm
